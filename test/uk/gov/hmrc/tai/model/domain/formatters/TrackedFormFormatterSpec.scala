  ...
  "trackingReads" should {
    "be able to parse incoming tracking" when {
      "the milestone is Received status" in {
        ...
        val tracking = parse(json).as[TrackedForm](trackedFormReads)
    ...
    "throw an exception" when{
      "formId doesn't exist in the json" in{
        ...
        val result = the[JsResultException] thrownBy parse(json).as[TrackedForm](trackedFormReads)
  ...
  "TrackingSequenceReads" should {
    "Return nil" when {
      "The list is empty" in {
        ...
        val trackingFormSequence = parse(json).as[Seq[TrackedForm]](trackedFormSeqReads)
